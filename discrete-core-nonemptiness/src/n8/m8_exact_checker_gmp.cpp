...
struct ExactLP{
    ...
    ExactLP(const std::vector<std::vector<long long>>&A,const std::vector<long long>&b,const std::vector<Rat>&c):m((int)b.size()),n((int)c.size()),B(m),Nv(n+1),D(m+2,std::vector<Rat>(n+2)){
...
struct Cell{
    ...
    Rat maxdot(const std::array<Rat,M>&p){
        ...
        int V=M-1;std::vector<Rat>obj(V);for(int c=0;c<V;++c)obj[c]=p[c]-p[M-1];ExactLP lp(C,b,obj);Rat z=p[M-1]*Rat(k)+lp.solve();cache.emplace(p,z);return z;
    }
};
...
static std::string ak(const std::array<Rat,N>&a){std::ostringstream o;for(auto&x:a)o<<x.get_num()<<'/'<<x.get_den()<<';';return o.str();}
...
static std::vector<Rat> singleton_vals(const Matrix&A,const std::array<int,N>&h,int k,int i,const Aff&P,Cell&X){
    ...
    for(auto const&a:D){std::array<Rat,M>p{};for(int c=0;c<M;++c)p[c]=a[i]*Rat((A.row[i]>>c)&1u);Rat z=Rat(k,8)*(Rat(1)-a[i])+a[i]*Rat(h[i])-X.maxdot(p);vals.push_back(z);}
    return vals;
}
static std::vector<Rat> adaptive_vals(const Matrix&A,const std::array<int,N>&h,int k,int E,const Aff&P,Cell&X){
    ...
    for(auto const&a:D){Rat sum=0,hh=0;std::array<Rat,M>p{};for(int i=0;i<N;++i)if((E>>i)&1u){sum+=a[i];hh+=a[i]*Rat(h[i]);for(int c=0;c<M;++c)if((A.row[i]>>c)&1u)p[c]+=a[i];}Rat z=Rat(k,8)*(Rat(es)-sum)+hh-X.maxdot(p);vals.push_back(z);}
    return vals;
}
static std::vector<Rat> coalvals(const Matrix&A,const std::array<int,N>&h,int k,int cm,int i,int S,const Aff&P,Cell&X){
    ...
    for(auto const&a:D){Rat con=0;std::array<Rat,M>p{};for(int j=0;j<N;++j)if((S>>j)&1u){con+=a[j]*Rat(q[j]+1);for(int c=0;c<M;++c)if((A.row[j]>>c)&1u)p[c]+=a[j];}vals.push_back(con-X.maxdot(p));}
    return vals;
}
static std::string rs(const Rat&x){std::ostringstream o;o<<x.get_num()<<"/"<<x.get_den();return o.str();}
